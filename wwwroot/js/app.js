...
function logout() {
  ...
  localStorage.removeItem('wt_progress');
...
function setActiveNav() {
  ...
  document.querySelectorAll('.nav-item').forEach(item => {
    ...
    if (!href) return;
    const target = href.split('/').pop().replace('.html', '').toLowerCase();
    item.classList.toggle('active', page === target);
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  else if (page === 'register' || page === 'register.html')  initRegister();
  else if (page === 'dashboard' || page === 'dashboard.html') initDashboard();
  else if (page === 'progress' || page === 'progress.html')  initProgress();
  else if (page === 'levels' || page === 'levels.html')    initLevels();
  setActiveNav();
});